...
| T5 过户资料回收 | 空中交付成本 | 50元 | 资料回收前车辆不在服务中心库/唯普库 | ❌ |
...
*For any* 车辆，在各时机点触发时，系统应按以下规则计入成本：T2检测过审时计入检测成本（省内85/省外120）；T3成交时计入评估费和销售成本（按车源类型分档），收购车另计入收购车成本（按车价分档）；T4交付型出库时计入现场交付成本150元；T5过户资料回收时，若过户费≠0则计入现场过户成本350元，若车辆不在服务中心库/唯普库则计入空中交付成本50元。
